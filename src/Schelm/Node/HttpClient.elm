...
type alias RawUrl = { request : String, diagnostic : String, origin : String }

type UrlError = InvalidUrl | UnsupportedScheme | UserInfoRejected | MissingHost

type Origin = Origin String

type OriginSet = OriginSet (List String)

type Method = Method String

type MethodError = InvalidMethod | ForbiddenMethod

type RequestHeader = RequestHeader String String

type RequestHeaderError = InvalidHeaderName | InvalidHeaderValue | ForbiddenHeader

type ResponseHeader = ResponseHeader String String

type Body = Empty | Utf8 Bytes | Binary Bytes

type ResponseLimit = ResponseLimit Int

type LimitError = LimitOutOfRange

type Timeout = Timeout Int

type TimeoutError = TimeoutOutOfRange

type Deadline = Deadline Float

type OverLimit = RejectOverLimit | TruncateOverLimit

type Request = Request RequestData
...
type Response = Response ResponseData
...
type Error = Error { kind : ErrorKind, site : String, code : Maybe String, message : String }
...
type alias RawError = { kind : String, site : String, code : String }
...
urlToRequestString (Url value) = value.request

urlForDiagnostic : Url -> String
urlForDiagnostic (Url value) = value.diagnostic

origin : Url -> Origin
origin (Url value) = Origin value.origin
...
method raw =
    let upper = String.toUpper raw in
    if String.isEmpty raw || not (List.all isTokenChar (String.toList raw)) then Err InvalidMethod
    else if upper == "CONNECT" || upper == "TRACE" then Err ForbiddenMethod
    else Ok (Method upper)

requestHeader : String -> String -> Result RequestHeaderError RequestHeader
requestHeader rawName value =
    let name = String.toLower rawName in
    if String.isEmpty name || not (List.all isTokenChar (String.toList name)) then Err InvalidHeaderName
    else if String.any (\c -> c == '\u{0000}' || c == '\u{000D}' || c == '\u{000A}') value then Err InvalidHeaderValue
    else if forbiddenHeader name then Err ForbiddenHeader
    else Ok (RequestHeader name value)

responseHeaderName : ResponseHeader -> String
responseHeaderName (ResponseHeader name _) = name

responseHeaderValue : ResponseHeader -> String
responseHeaderValue (ResponseHeader _ value) = value
...
utf8Body value =
    let bytes = BytesEncode.encode (BytesEncode.string value) in
    boundedBody Utf8 bytes

bytesBody : Bytes -> Result RequestError Body
bytesBody value = boundedBody Binary (Elm.Kernel.SchelmHttp.copyBytes value)

bodyByteLength : Body -> Int
bodyByteLength value =
    case value of
        Empty -> 0
        Utf8 bytes -> Bytes.width bytes
        Binary bytes -> Bytes.width bytes

responseLimit : Int -> Result LimitError ResponseLimit
responseLimit value =
    if value >= 1 && value <= 8 * 1024 * 1024 then Ok (ResponseLimit value) else Err LimitOutOfRange

timeout : Int -> Result TimeoutError Timeout
timeout value =
    if value >= 1 && value <= 120000 then Ok (Timeout value) else Err TimeoutOutOfRange
...
request config =
    let
        (Method methodValue) = config.method
        (Url urlValue) = config.url
        (ResponseLimit limitValue) = config.responseLimit
        bodyWidth = bodyByteLength config.body
    in
...
status : Response -> Int
status (Response value) = value.status

statusText : Response -> String
statusText (Response value) = value.statusText

headers : Response -> List ResponseHeader
headers (Response value) = value.headers

setCookies : Response -> List String
setCookies (Response value) = value.setCookies

responseUrl : Response -> Url
responseUrl (Response value) = value.url

responseBody : Response -> ResponseBody
responseBody (Response value) = value.body

errorKind : Error -> ErrorKind
errorKind (Error value) = value.kind

errorMessage : Error -> String
errorMessage (Error value) = value.message

errorCode : Error -> Maybe String
errorCode (Error value) = value.code
...
boundedBody ctor bytes =
    if Bytes.width bytes > 8 * 1024 * 1024 then Err RequestBodyTooLarge else Ok (ctor bytes)

bodyBytes : Body -> Bytes
bodyBytes value =
    case value of
        Empty -> BytesEncode.encode (BytesEncode.sequence [])
        Utf8 bytes -> bytes
        Binary bytes -> bytes

decodeUrl : { ok : Bool, reason : String, value : RawUrl } -> Result UrlError Url
decodeUrl raw =
    if raw.ok then Ok (Url raw.value)
    else case raw.reason of
        "scheme" -> Err UnsupportedScheme
        "userinfo" -> Err UserInfoRejected
        "host" -> Err MissingHost
        _ -> Err InvalidUrl
    ...
    let
        decodedBody =
            case raw.bodyKind of
                "truncated" -> Truncated { prefix = raw.body, decodedLengthAtLeast = raw.decodedLengthAtLeast }
                "discarded-redirect" -> DiscardedRedirectBody
                _ -> Complete raw.body
    in
    Response
        { status = raw.status
...
decodeError raw =
    let
        kind = case raw.kind of
            ...
            _ -> UnknownFailure
        code = if String.isEmpty raw.code then Nothing else Just raw.code
        message = errorKindText kind ++ " at " ++ raw.site
    in
...
errorKindText kind =
    case kind of
        InvalidRequestError -> "invalid HTTP request"
        OriginNotAllowed -> "HTTP origin not allowed"
        DeadlineExceeded -> "HTTP deadline exceeded"
        ResponseTooLarge -> "HTTP response exceeded decoded byte limit"
        NetworkFailure -> "HTTP network failure"
        UnsupportedRuntime -> "unsupported Node Fetch runtime"
        UnknownFailure -> "unknown HTTP failure"

unique : List String -> List String
unique values = List.foldr (\value acc -> if List.any ((==) value) acc then acc else value :: acc) [] values
...
forbiddenHeader name =
    List.any ((==) name)
        [ "connection", "content-length", "host", "keep-alive", "te", "trailer"
        , "transfer-encoding", "upgrade", "cookie", "set-cookie", "authorization"
