...
const root=path.resolve(__dirname,".."),tool=JSON.parse(fs.readFileSync(path.join(root,"toolchain.json"),"utf8"));
const archive=path.join(root,"vendor/toolchain/public-packages-0.19.2.tar.gz");
...
function install(name,source,entries){const [author,project]=name.split("/"),dest=path.join(packages,author,project,"1.1.0");fs.mkdirSync(dest,{recursive:true});for(const entry of entries)fs.cpSync(path.join(source,entry),path.join(dest,entry),{recursive:true});}
install("sjalq/schelm-node-http-client",root,["src","elm.json","README.md","LICENSE"]);
