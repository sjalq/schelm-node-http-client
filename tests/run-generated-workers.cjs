...
function server(){return new Promise(resolve=>{const s=http.createServer((req,res)=>{if(req.url==="/echo"){const parts=[];req.on("data",c=>parts.push(c));req.on("end",()=>{const b=Buffer.concat(parts);res.end(JSON.stringify({method:req.method,length:b.length}));});return;}if(req.url==="/slow"){setTimeout(()=>res.end("late"),500);return;}if(req.url==="/gzip"||req.url==="/deflate"||req.url==="/br"){const plain=Buffer.alloc(4096,65),kind=req.url.slice(1),b=kind==="gzip"?zlib.gzipSync(plain):kind==="deflate"?zlib.deflateSync(plain):zlib.brotliCompressSync(plain);res.writeHead(200,{"content-encoding":kind,"content-length":b.length});res.end(b);return;}if(req.url==="/redirect"){res.writeHead(301,{location:"/ok"});res.end(Buffer.alloc(10000));return;}res.writeHead(req.url==="/empty"?204:200,{"set-cookie":["a=1","b=2"],"x-dupe":["one","two"]});res.end(req.url==="/large"?Buffer.alloc(64,7):Buffer.from("ok"));});s.listen(0,"127.0.0.1",()=>resolve(s));});}
