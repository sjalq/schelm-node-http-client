...
(async()=>{
 ...
 try{
  for(const mode of ["debug","optimize"]){
   ...
   const ergonomicFile=path.join("build",`ergonomics-${mode}.js`);
   let ergonomic=await run(ergonomicFile,"ErgonomicsWorker",{url:base+"/json",mode:"json"});assert.equal(ergonomic.reports[0].ok,"Elm");
   for(const status of [301,302,303,307,308]){ergonomic=await run(ergonomicFile,"ErgonomicsWorker",{url:base+`/redirect-${status}`,mode:"redirect"});assert.equal(ergonomic.reports[0].ok,"Elm",`debug/opt redirect ${status} follows`);}
   for(const status of [200,300,304,305,306]){ergonomic=await run(ergonomicFile,"ErgonomicsWorker",{url:base+`/redirect-${status}`,mode:"redirect"});assert.equal(ergonomic.reports[0].ok,`not-followed-${status}`,`debug/opt status ${status} does not follow`);}
   ergonomic=await run(ergonomicFile,"ErgonomicsWorker",{url:base+"/echo-json",mode:"post-json"});assert.equal(ergonomic.reports[0].ok,'{"name":"Elm"}');
   ergonomic=await run(ergonomicFile,"ErgonomicsWorker",{url:base+"/bad-utf8",mode:"text"});assert.equal(ergonomic.reports[0].error,"invalid-utf8");
   ...
   r=await run(file,"HttpWorker",{url:base+"/redirect-301",limit:8,timeoutMs:2000,truncate:false,discardRedirectBody:true});assert.equal(r.reports[0].bodyKind,"discarded-redirect");
