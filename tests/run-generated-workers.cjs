...
(async()=>{
 ...
 try{
  for(const mode of ["debug","optimize"]){
   ...
   ergonomic=await run(ergonomicFile,"ErgonomicsWorker",{url:base+"/redirect",mode:"redirect"});assert.equal(ergonomic.reports[0].ok,"Elm");
